...
def test_readme_names_the_real_entrypoint():
    assert "python -m core" in README
    assert (pathlib.Path(__file__).resolve().parents[1] / "core" / "__main__.py").exists()


def test_every_variable_the_readme_lists_is_read_by_config():
    listed = set(re.findall(r"`(TRINO_[A-Z_]+|MCP_[A-Z_]+)`", README))
    source = (pathlib.Path(__file__).resolve().parents[1] / "core" / "config.py").read_text(encoding="utf-8")
...
def test_readme_defaults_match_the_code(monkeypatch):
    for name in ("MCP_PORT", "MCP_HEALTH_PORT", "MCP_TRANSPORT", "TRINO_MAX_ROWS", "TRINO_READ_ONLY"):
