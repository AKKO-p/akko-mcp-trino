...
- Tool plugins: a package registers a registrar under the entry-point group `akko_mcp_trino.tools` and the operator names it in `MCP_TOOL_PLUGINS`; `--check` validates the names. This is how a product adds its own tools to the image without forking the server.
